...
class TagInStore(MethodView):
    ...
    def post(self, tag_data, store_id):
        # Can check for the tag already present for this store
        # Not needed as we specified unique name in the model
        if TagModel.query.filter(TagModel.store_id == store_id, TagModel.name == tag_data["name"]).first():
            abort(400, message=f"Tag {tag_data['name']} already exists for store {store_id}")
        ...
        try:
            ...
            abort(
                500, message=f"Failed to insert tag for store {store_id} with {str(e)}"
            )
...
class LinkTagsToItem(MethodView):
    ...
    def post(self, item_id, tag_id):
        ...
        try:
            ...
            abort(500, message=f"A {type(e)} error occured while inserting the tag {tag_id} ({str(e)})")
...
class Tag(MethodView):
    ...
    @blp.response(202, description="Deletes a tag if no item is tagged with it.", example={"message": "Tag deleted."}, schema=TagSchema)
